      ...
      >
        <template #item="{ element: option }">
          <div class="option-row">
            ...
            ></v-text-field>
            <v-btn icon size="small" color="error" @click="removeOption(option)">
...
const options = ref(
  (props.element.content.options || []).map((text: string) => ({ id: crypto.randomUUID(), text }))
)
...
watch(() => props.element, (newValue) => {
  ...
  options.value = (newValue.content.options || []).map((text: string) => ({ id: crypto.randomUUID(), text }))
